...
# DISCLAIMER: this script was somewhat specific to my data - all of the private alleles in my populations were due to one popultion being biallelic at a locus with all others were monoallelic, so this script DOES NOT WORK WITH FIXED DIFFERENCES among populations
...
# First run populations on ALL of your populations with the -p (--min-populations) option set to the total number of populations. Create a whitelist from this run (tail -n +7 populations.sumstats.tsv | cut -f 1,4 | uniq > whitelist.tsv) to feed into the second run of populations; this run will be refered to as "all populations" or "all"
